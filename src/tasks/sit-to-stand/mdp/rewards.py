...
def base_height(
  ...
  """Reward reaching target base height."""
  asset: Entity = env.scene[asset_cfg.name]
  height = asset.data.root_link_pos_w[:, 2]
  return torch.clamp(height - target_height, min=0.0)
...
def base_height_progress(
  ...
  """Reward increase in base height between steps."""
  asset: Entity = env.scene[asset_cfg.name]
  height = asset.data.root_link_pos_w[:, 2]

  if not hasattr(env, "_prev_height"):
    env._prev_height = height.clone()

  progress = height - env._prev_height
  env._prev_height = height.clone()
...
def upright_reward(
  ...
  """Reward upright orientation using projected gravity."""
  asset: Entity = env.scene[asset_cfg.name]
  g = asset.data.projected_gravity_b  # [B, 3]

  target = torch.tensor([0.0, 0.0, -1.0], device=g.device)
  error = torch.norm(g - target, dim=1)
...
def stability_reward(
  ...
  """Reward low angular velocity (stability)."""
  asset: Entity = env.scene[asset_cfg.name]
  ang_vel = asset.data.root_link_ang_vel_b

  return torch.exp(-0.5 * torch.sum(torch.square(ang_vel), dim=1))


def stand_still_after_standing(
  env: ManagerBasedRlEnv,
  target_height: float,
  asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
  """Encourage staying still once standing."""
  asset: Entity = env.scene[asset_cfg.name]

  height = asset.data.root_link_pos_w[:, 2]
  joint_vel = asset.data.joint_vel[:, asset_cfg.joint_ids]

  standing = (height > target_height).float()
  vel_penalty = torch.sum(torch.square(joint_vel), dim=1)

  return -standing * vel_penalty